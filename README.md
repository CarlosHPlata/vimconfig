...
and goals behind the chosen Software Architecture used by Process Tempo.
In this wiki are presented some of the techniques, best practices,
architectural patterns, and guidelines followed gathered by the team.

This wiki does not have the goal of teaching how to build and create a
project from scratch, for that you can follow the information on the
Backend Architecture Guideline and Frontend Architecture Guideline
rather this Wiki will provide you with all the technicity and knowledge
which inspired the architecture behind the code of Process Tempo.

Code examples are written using TypeScript.

## Clean Architecture

![Clean Architecture Flow Diagram](Image.png
"Clean Architecture Flow Diagram") Clean Architecture is a software
...
<u>To start we have first to look at the Clean Architecture flow
diagram.</u>
...
Domain should know nothing about what is in the Application Layer.
  ...
  - **Infrastructure:** This layer represents all the technical
    ...
    *the controllers of an HTTP Framework, the Components from Angular,
    the ORM implementation of a DB Connection.* This layer also contains
...
![Hexagonal Architecture Figure 1](Hexagonal_Architecture_figure_1.png
"Hexagonal Architecture Figure 1") The hexagonal architecture, or ports
...
![Hexagonal\_Architecture\_Figure\_2.png](Hexagonal_Architecture_Figure_2.png
"Hexagonal_Architecture_Figure_2.png")
...
In Process Tempo we divide the clean architecture by Modules. Each
...
implementations are set on the Infrastructure layer.
  ...
  - Ports are tied to communication between services, events, or other
    modules
  - Adapters\[<https://en.wikipedia.org/wiki/Adapter_pattern#>:\~:text=In%20software%20engineering%2C%20the%20adapter,be%20used%20as%20another%20interface.\]
    are contracts to communicate to some intrusive libraries to
...
(Dependency
Injection\[<https://en.wikipedia.org/wiki/Dependency_injection#>:\~:text=In%20software%20engineering%2C%20dependency%20injection,other%20objects%20are%20called%20dependencies.\&text=The%20%22injection%22%20refers%20to%20the,client)%20that%20would%20use%20it.\]
and Dependency
...
For example:
  ...
  - On the other hand, programmer error means something unexpected
    ...
    should be logged and then returned to the user as `500 Internal
    Server Error`, in this case without adding much additional details
